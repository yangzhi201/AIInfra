...
表1 的数据显示，在6.7B规模以下，Zeropoint量化（非对称）的性能始终优于Absmax量化（对称）。表3统计了具有不对称分布（即只取正値或只取负值）的异常特征数量，发现几乎所有的异常值都具有严格的不对称分布(单向的)。因此可推断，异常特征的不对称性是导致对称量化方法（如Absmax）早期失效的原因。
